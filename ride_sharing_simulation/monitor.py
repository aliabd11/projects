...
class Monitor:
    ...
    def _average_total_distance(self):
        ...
        counter = 0
        location_list = []
        for activities in self._activities[DRIVER].values(): #for activities for each driver
            if len(activities) > 1: #if greater than 1, meaning driver isn't stationary
                for activity in activities: #for each location the driver travels
                    location_list.append(activity.location)
            while counter < len(location_list) - 1: #calculate the total distances the driver travels
                total_distances += manhattan_distance(location_list[counter], location_list[counter+1]) #add to total
                counter += 1
            location_list = [] #reset values for next driver
            counter = 0

        return total_distances / len(self._activities[DRIVER])
