...
class Simulation:
    ...
    def run(self, initial_events):
        ...
        while not self._events.is_empty():
            ...
            returned_events = event.do(self._dispatcher, self._monitor) #Runs removed event
            if returned_events is not None: #if returned event is not None ("empty")
                if type(returned_events) is list: #if it's a list, loop through each element
                    for each_element in returned_events:
                        self._events.add(each_element) #add each element back into queue
                else: #else if not list, add to queue
                    self._events.add(returned_events)
