...
public class FileChooserButtonListener implements ActionListener {
	...
	private static void buildTree(File file, FileNode curr) {
		...
		for (File child : directoryListing) {
			FileNode childNode = new FileNode(child.getName(), null, FileType.FILE);
			curr.addChild(child.getName(), childNode);
			if (child.isDirectory()) {
				FileNode dirChildNode = new FileNode(child.getName(), null, FileType.DIRECTORY);
				curr.addChild(child.getName(), dirChildNode);
				buildTree(child, dirChildNode);
			}
		}
	}
	...
	private static void buildDirectoryContents(FileNode fileNode, StringBuffer contents, String prefix) {		
		...
		for (FileNode value : children) {
			if (value.isDirectory() == true) {
				buildDirectoryContents(value, contents, prefix + DirectoryExplorer.PREFIX);
			} else {
				contents.append(prefix + DirectoryExplorer.PREFIX);
				contents.append(value.getName() + "\n"); 
				}	
			} 
		}
	}
