...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Checks if there is one open position that has no symbols available to
        put in. If true, the solution will fail.
        @rtype: Boolean
        ...
        False
        """
        for position in range(self._n**2):
            if self._symbols[position] == "*":
                surround_set = self._row_set(position) | self._column_set(position) | \
                      self._subsquare_set(position)  # Union of all three
                if surround_set >= self._symbol_set:
                    return True
        return False
